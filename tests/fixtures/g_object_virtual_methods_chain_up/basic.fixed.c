...
// Invalid: constructed missing chain-up

typedef struct _Qux Qux;
G_DECLARE_FINAL_TYPE (Qux, qux, MY, QUX, GObject)
struct _Qux { GObject parent_instance; gint value; };
G_DEFINE_TYPE (Qux, qux, G_TYPE_OBJECT)

static void qux_init (Qux *self) { }
...
static void
qux_class_init (QuxClass *klass)
{
  G_OBJECT_CLASS (klass)->constructed = qux_constructed;
}

// Valid: constructed chains up

typedef struct _Waldo Waldo;
G_DECLARE_FINAL_TYPE (Waldo, waldo, MY, WALDO, GObject)
struct _Waldo { GObject parent_instance; gint value; };
G_DEFINE_TYPE (Waldo, waldo, G_TYPE_OBJECT)

static void waldo_init (Waldo *self) { }

static void
waldo_constructed (GObject *object)
{
  Waldo *self = MY_WALDO (object);

  G_OBJECT_CLASS (waldo_parent_class)->constructed (object);

  self->value = 42;
}

static void
waldo_class_init (WaldoClass *klass)
{
  G_OBJECT_CLASS (klass)->constructed = waldo_constructed;
}
